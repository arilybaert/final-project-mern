...
import { IGameDay, IGame, GameDay } from '../../models/mongoose';

class GameDaySeeder {

    public GAMES_PREFIX_URL: string;
    public GAMES_SUFFIX_URL: string;

    constructor() {
        this.GAMES_PREFIX_URL = "http://data.nba.net/10s/prod/v1/";
        this.GAMES_SUFFIX_URL = "/scoreboard.json";
    }
    ...
    createGamedays = async (date: any, id ? : any) => {
        ...
        await GameDay.countDocuments({
            ...
            if (count > 0) {
                ...
                await GameDay.findOneAndUpdate({
                        _id: id
                    }, temp, {
                        new: true,
                        upsert: false
    ...
    getGames = async (date: string) => {
        const url = `${this.GAMES_PREFIX_URL}${date}${this.GAMES_SUFFIX_URL}`;
